...
std::vector<cv::Rect> detectFaceLocation(cv::Mat& frame)
{
    ...
    seeta::FaceDetection detector("UseFaceDetection/model/seeta_fd_frontal_v1.0.bin");
...
//        #endif
//
//        std::vector<cv::Rect> face_rects = detectFaceLocation(frame);
//
//        for (int32_t i = 0; i < face_rects.size(); i++) {
//            cv::rectangle(frame, face_rects[i], CV_RGB(250, 230, 215), 4, 8, 0);
//        }
//
